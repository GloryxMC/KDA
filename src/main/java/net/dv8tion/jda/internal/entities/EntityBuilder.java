...
public class EntityBuilder {
    ...
    public SelfUser createSelfUser(DataObject self) {
        ...
        if (!self.isNull("application_id")) selfUser.setApplicationId(self.getUnsignedLong("application_id"));
    ...
    public static EmojiUnion createEmoji(DataObject emoji) {
        if (emoji.isNull("id")) return new UnicodeEmojiImpl(emoji.getString("name"));
    ...
    private void createGuildStickerPass(GuildImpl guildObj, DataArray array) {
        ...
        try (UnlockHook hook = stickerView.writeLock()) {
            ...
            for (int i = 0; i < array.length(); i++) {
                DataObject object = array.getObject(i);
                if (object.isNull("id")) {
                    LOG.error("Received GUILD_CREATE with a sticker with a null ID. GuildId: {} JSON: {}", guildObj.getId(), object);
                    continue;
                }
                if (object.getInt("type", -1) != Sticker.Type.GUILD.getId()) {
                    LOG.error("Received GUILD_CREATE with sticker that had an unexpected type. GuildId: {} Type: {} JSON: {}", guildObj.getId(), object.getInt("type", -1), object);
                    continue;
                }
    ...
    public GuildImpl createGuild(long guildId, DataObject guildJson, TLongObjectMap<DataObject> members, int memberCount) {
        ...
        try (UnlockHook h1 = guildObj.getMembersView().writeLock(); UnlockHook h2 = getJDA().getUsersView().writeLock()) {
        ...
        if (guildObj.getMember(api.getSelfUser()) == null) {
            ...
            guildObj.retrieveMembersByIds(api.getSelfUser().getIdLong()).onSuccess(m -> {
                ...
                else LOG.debug("Successfully recovered SelfMember for guild with id {}.", guildId);
            });
        }
    ...
    public UserImpl createUser(DataObject user) {
        ...
        User.Profile profile = user.hasKey("banner") ? new User.Profile(id, user.getString("banner", null), user.getInt("accent_color", User.DEFAULT_ACCENT_COLOR_RAW)) : null;

        if (newUser) {
            // Initial creation
            userObj.setName(user.getString("username")).setDiscriminator(user.get("discriminator").toString()).setAvatarId(user.getString("avatar", null)).setBot(user.getBoolean("bot")).setSystem(user.getBoolean("system")).setFlags(user.getInt("public_flags", 0)).setProfile(profile);
        } else {
            // Fire update events
    ...
    public boolean updateMemberCache(MemberImpl member, boolean forceRemove) {
        ...
        if (forceRemove || !getJDA().cacheMember(member)) {
            ...
            if (voiceState != null) {
                ...
                if (connectedChannel != null) connectedChannel.getConnectedMembersMap().remove(member.getIdLong());
    ...
    public void updateMember(GuildImpl guild, MemberImpl member, DataObject content, List<Role> newRoles) {
        ...
        if (content.hasKey("nick")) {
            ...
            if (!Objects.equals(oldNick, newNick)) {
                ...
                getJDA().handleEvent(new GuildMemberUpdateNicknameEvent(getJDA(), responseNumber, member, oldNick));
            }
        }
        ...
        if (content.hasKey("premium_since")) {
            long epoch = 0;
            if (!content.isNull("premium_since")) epoch = Helpers.toTimestamp(content.getString("premium_since"));
            if (epoch != member.getBoostDateRaw()) {
                OffsetDateTime oldTime = member.getTimeBoosted();
                member.setBoostDate(epoch);
                getJDA().handleEvent(new GuildMemberUpdateBoostTimeEvent(getJDA(), responseNumber, member, oldTime));
            }
        }
        ...
        if (content.hasKey("communication_disabled_until")) {
            ...
            if (epoch != member.getTimeOutEndRaw()) {
                ...
                getJDA().handleEvent(new GuildMemberUpdateTimeOutEvent(getJDA(), responseNumber, member, oldTime));
            }
        }
        ...
        if (!content.isNull("pending")) {
            ...
            if (pending != oldPending) {
                ...
                getJDA().handleEvent(new GuildMemberUpdatePendingEvent(getJDA(), responseNumber, member, oldPending));
            }
        }
    ...
    public Category createCategory(GuildImpl guild, DataObject json, long guildId) {
        ...
        if (channel == null) {
            ...
            SnowflakeCacheViewImpl<Category> guildCategoryView = guild.getCategoriesView(), categoryView = getJDA().getCategoriesView();
            try (UnlockHook glock = guildCategoryView.writeLock(); UnlockHook jlock = categoryView.writeLock()) {
    ...
    public TextChannel createTextChannel(GuildImpl guildObj, DataObject json, long guildId) {
        ...
        if (channel == null) {
            ...
            SnowflakeCacheViewImpl<TextChannel> guildTextView = guildObj.getTextChannelsView(), textView = getJDA().getTextChannelsView();
            try (UnlockHook glock = guildTextView.writeLock(); UnlockHook jlock = textView.writeLock()) {
        ...
        channel.setParentCategory(json.getLong("parent_id", 0)).setLatestMessageIdLong(json.getLong("last_message_id", 0)).setName(json.getString("name")).setTopic(json.getString("topic", null)).setPosition(json.getInt("position")).setNSFW(json.getBoolean("nsfw")).setSlowmode(json.getInt("rate_limit_per_user", 0));
    ...
    public NewsChannel createNewsChannel(GuildImpl guildObj, DataObject json, long guildId) {
        ...
        if (channel == null) {
            if (guildObj == null) guildObj = (GuildImpl) getJDA().getGuildsView().get(guildId);
            SnowflakeCacheViewImpl<NewsChannel> guildNewsView = guildObj.getNewsChannelView(), newsView = getJDA().getNewsChannelView();
            try (UnlockHook glock = guildNewsView.writeLock(); UnlockHook jlock = newsView.writeLock()) {
        ...
        channel.setParentCategory(json.getLong("parent_id", 0)).setLatestMessageIdLong(json.getLong("last_message_id", 0)).setName(json.getString("name")).setTopic(json.getString("topic", null)).setPosition(json.getInt("position")).setNSFW(json.getBoolean("nsfw"));
    ...
    public VoiceChannel createVoiceChannel(GuildImpl guild, DataObject json, long guildId) {
        ...
        if (channel == null) {
            if (guild == null) guild = (GuildImpl) getJDA().getGuildsView().get(guildId);
            SnowflakeCacheViewImpl<VoiceChannel> guildVoiceView = guild.getVoiceChannelsView(), voiceView = getJDA().getVoiceChannelsView();
            try (UnlockHook vlock = guildVoiceView.writeLock(); UnlockHook jlock = voiceView.writeLock()) {
        ...
        channel.setParentCategory(json.getLong("parent_id", 0)).setLatestMessageIdLong(json.getLong("last_message_id", 0)).setName(json.getString("name")).setPosition(json.getInt("position")).setUserLimit(json.getInt("user_limit")).setNSFW(json.getBoolean("nsfw")).setBitrate(json.getInt("bitrate")).setRegion(json.getString("rtc_region", null));
    ...
    public StageChannel createStageChannel(GuildImpl guild, DataObject json, long guildId) {
        ...
        if (channel == null) {
            if (guild == null) guild = (GuildImpl) getJDA().getGuildsView().get(guildId);
            SnowflakeCacheViewImpl<StageChannel> guildStageView = guild.getStageChannelsView(), stageView = getJDA().getStageChannelView();
            try (UnlockHook vlock = guildStageView.writeLock(); UnlockHook jlock = stageView.writeLock()) {
        ...
        channel.setParentCategory(json.getLong("parent_id", 0)).setName(json.getString("name")).setPosition(json.getInt("position")).setBitrate(json.getInt("bitrate")).setRegion(json.getString("rtc_region", null));
    ...
    public ThreadChannel createThreadChannel(GuildImpl guild, DataObject json, long guildId) {
        ...
        if (channel == null) {
            SnowflakeCacheViewImpl<ThreadChannel> guildThreadView = guild.getThreadChannelsView(), threadView = getJDA().getThreadChannelsView();
            try (UnlockHook vlock = guildThreadView.writeLock(); UnlockHook jlock = threadView.writeLock()) {
        ...
        channel.setName(json.getString("name")).setParentChannelId(json.getLong("parent_id")).setOwnerId(json.getLong("owner_id")).setMemberCount(json.getInt("member_count")).setMessageCount(json.getInt("message_count")).setLatestMessageIdLong(json.getLong("last_message_id", 0)).setSlowmode(json.getInt("rate_limit_per_user", 0)).setLocked(threadMetadata.getBoolean("locked")).setArchived(threadMetadata.getBoolean("archived")).setInvitable(threadMetadata.getBoolean("invitable")).setArchiveTimestamp(Helpers.toTimestamp(threadMetadata.getString("archive_timestamp"))).setCreationTimestamp(threadMetadata.isNull("create_timestamp") ? 0 : Helpers.toTimestamp(threadMetadata.getString("create_timestamp"))).setAutoArchiveDuration(ThreadChannel.AutoArchiveDuration.fromKey(threadMetadata.getInt("auto_archive_duration")));
    ...
    public ThreadMember createThreadMember(ThreadChannelImpl threadChannel, Member member, DataObject json) {
        ...
        threadMember.setJoinedTimestamp(Helpers.toTimestamp(json.getString("join_timestamp"))).setFlags(json.getInt("flags"));
    ...
    public PrivateChannel createPrivateChannel(DataObject json, UserImpl user) {
        ...
        if (channel == null) {
            channel = new PrivateChannelImpl(getJDA(), channelId, user).setLatestMessageIdLong(json.getLong("last_message_id", 0));
        }
    ...
    public ReceivedMessage createMessageWithChannel(DataObject json, @NotNull MessageChannel channel, boolean modifyCache) {
        ...
        if (channel instanceof GuildMessageChannel) return createMessage0(json, channel, modifyCache);
        // Try to resolve private channel recipient if needed
        if (channel instanceof PrivateChannel) return createMessageWithLookup(json, null, modifyCache);
    ...
    public ReceivedMessage createMessageWithLookup(DataObject json, @Nullable Guild guild, boolean modifyCache) {
        ...
        if (guild == null) return createMessage0(json, createPrivateChannelByMessage(json), modifyCache);
    ...
    private PrivateChannel createPrivateChannelByMessage(DataObject message) {
        ...
        if (channel == null) {
            ...
            if (!isAuthorSelfUser) channelData.put("recipient", author);
    ...
    private ReceivedMessage createMessage0(DataObject jsonObject, @NotNull MessageChannel channel, boolean modifyCache) {
        ...
        if (channel instanceof GuildChannel) guild = (GuildImpl) ((GuildChannel) channel).getGuild();
        ...
        Mentions mentions = new MessageMentionsImpl(api, guild, content, mentionsEveryone, jsonObject.getArray("mentions"), jsonObject.getArray("mention_roles"));
        ...
        if (!type.isSystem()) {
            return new ReceivedMessage(id, channel, type, messageReference, fromWebhook, tts, pinned, content, nonce, user, member, activity, editTime, mentions, reactions, attachments, embeds, stickers, components, flags, messageInteraction, startedThread);
        } else {
            return new SystemMessage(id, channel, type, messageReference, fromWebhook, tts, pinned, content, nonce, user, member, activity, editTime, mentions, reactions, attachments, embeds, stickers, flags, startedThread);
        }
    }
    ...
    public MessageEmbed createMessageEmbed(DataObject content) {
        ...
        if (content.isNull("thumbnail")) {
            ...
            thumbnail = new Thumbnail(obj.getString("url", null), obj.getString("proxy_url", null), obj.getInt("width", -1), obj.getInt("height", -1));
        }
        ...
        if (content.isNull("author")) {
            ...
            author = new AuthorInfo(obj.getString("name", null), obj.getString("url", null), obj.getString("icon_url", null), obj.getString("proxy_icon_url", null));
        }
        ...
        if (content.isNull("video")) {
            ...
            video = new VideoInfo(obj.getString("url", null), obj.getInt("width", -1), obj.getInt("height", -1));
        }
        ...
        if (content.isNull("footer")) {
            ...
            footer = new Footer(obj.getString("text", null), obj.getString("icon_url", null), obj.getString("proxy_icon_url", null));
        }
        ...
        if (content.isNull("image")) {
            ...
            image = new ImageInfo(obj.getString("url", null), obj.getString("proxy_url", null), obj.getInt("width", -1), obj.getInt("height", -1));
        }

        final List<Field> fields = map(content, "fields", (obj) -> new Field(obj.getString("name", null), obj.getString("value", null), obj.getBoolean("inline", false)));

        return createMessageEmbed(url, title, description, type, timestamp, color, thumbnail, provider, author, video, footer, image, fields);
    }

    public static MessageEmbed createMessageEmbed(String url, String title, String description, EmbedType type, OffsetDateTime timestamp, int color, Thumbnail thumbnail, Provider siteProvider, AuthorInfo author, VideoInfo videoInfo, Footer footer, ImageInfo image, List<Field> fields) {
    ...
    public WebhookImpl createWebhook(DataObject object, boolean allowMissingChannel) {
        ...
        WebhookImpl webhook = new WebhookImpl(channel, getJDA(), id, type).setToken(token).setOwner(ownerMember, owner).setUser(defaultUser);
    ...
    public Invite createInvite(DataObject object) {
        ...
        return new InviteImpl(getJDA(), code, expanded, inviter, maxAge, maxUses, temporary, timeCreated, uses, channel, guild, group, target, type);
    }
    ...
    public ApplicationInfo createApplicationInfo(DataObject object) {
        ...
        final List<String> tags = object.optArray("tags").orElseGet(DataArray::empty).stream(DataArray::getString).collect(Collectors.toList());

        final Optional<DataObject> installParams = object.optObject("install_params");

        final long defaultAuthUrlPerms = installParams.map(o -> o.getLong("permissions")).orElse(0L);

        final List<String> defaultAuthUrlScopes = installParams.map(obj -> obj.getArray("scopes").stream(DataArray::getString).collect(Collectors.toList())).orElse(Collections.emptyList());

        return new ApplicationInfoImpl(getJDA(), description, doesBotRequireCodeGrant, iconId, id, flags, isBotPublic, name, termsOfServiceUrl, privacyPolicyUrl, owner, team, tags, customAuthUrl, defaultAuthUrlPerms, defaultAuthUrlScopes);
    }
