...
class CommandClient(private val prefixProvider: PrefixProvider, private val cooldownProvider: CooldownProvider, private val ignoreBots: Boolean, private val eventListeners: List<CommandEventAdapter>, private val commandExecutor: ExecutorService?, customOwnerIds: MutableSet<Long>) : EventListener {
    ...
    private fun onMessageReceived(event: MessageReceivedEvent) {
        ...
        val trigger = prefixes.firstOrNull { content.startsWith(it) } // This will break for "?", "??", "???"
                ?: return
        ...
        if (cooldown != null) {
            ...
            val entityId = when (cooldown.bucket) {
            ...
            if (entityId != null) {
                if (cooldownProvider.isOnCooldown(entityId, cooldown.bucket, cmd)) {
                    val time = cooldownProvider.getCooldownTime(entityId, cooldown.bucket, cmd)
        ...
        val shouldExecute = eventListeners.all { it.onCommandPreInvoke(ctx, cmd) } && cmd.cog.localCheck(ctx, cmd)
        ...
        val arguments: HashMap<KParameter, Any?> = try {
            ArgParser.parseArguments(exc, ctx, args, cmd.properties.argDelimiter)
        } catch (e: BadArgument) {
        ...
        if (cooldown != null && cooldown.duration > 0) {
            val entityId = when (cooldown.bucket) {
                BucketType.USER -> ctx.author.idLong
                BucketType.GUILD -> ctx.guild?.idLong
                BucketType.GLOBAL -> -1
            ...
            if (entityId != null) {
                val time = cooldown.timeUnit.toMillis(cooldown.duration)
                cooldownProvider.setCooldown(entityId, cooldown.bucket, time, cmd)
            }
        }
