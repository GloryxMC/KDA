...
class DataArray(val data: MutableList<Any>?) : Iterable<Any?>, SerializableArray {
    ...
    fun getObject(index: Int): DataObject {
        ...
        try {
            child = get(MutableMap::class.java, index) as MutableMap<String?, Any?>?
    ...
    fun getBoolean(index: Int, defaultValue: Boolean): Boolean {
        val value = get(Boolean::class.java, index, { s: String? -> java.lang.Boolean.parseBoolean(s) }, null)
        return value ?: defaultValue
    }
    ...
    private operator fun <T> get(
        @Nonnull type: Class<T>, index: Int, stringMapper: Function<String, T>?, numberMapper: Function<Number, T>?
    ): T? {
        ...
        throw ParsingException(
            Helpers.format(
                ...
                index, type.simpleName, value, value.javaClass.simpleName
            )
        )
    }
    ...
    companion object {
        ...
        fun fromETF(@Nonnull data: ByteArray?): DataArray {
            ...
            return try {
                val list = ExTermDecoder.unpackList(ByteBuffer.wrap(data))
                DataArray(list)
            } catch (ex: Exception) {
