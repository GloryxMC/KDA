...
object ExTermDecoder {
     ...
     */
    fun unpackMap(buffer: ByteBuffer): MutableMap<String, Any>? {
        val tag = buffer[1]
        require(tag == ExTermTag.MAP) { "Cannot unpack map from tag $tag" }
        return unpack(buffer) as MutableMap<String, Any>?
    }
     ...
     */
    fun unpackList(buffer: ByteBuffer): MutableList<Any>? {
        val tag = buffer[1]
        require(tag == ExTermTag.LIST) { "Cannot unpack list from tag $tag" }
        return unpack(buffer) as MutableList<Any>?
    }
    ...
    private fun unpackString(buffer: ByteBuffer): List<Any> {
        var length = java.lang.Short.toUnsignedInt(buffer.short)
    ...
    private fun unpackAtom(buffer: ByteBuffer, charset: Charset): Any? {
        val length = java.lang.Short.toUnsignedInt(buffer.short)
        return unpackAtom(buffer, charset, length)
    }
    ...
    private fun unpackMap0(buffer: ByteBuffer): Map<String?, Any?> {
        ...
        var arity = buffer.int
        while (arity-- > 0) {
            val key = unpack0(buffer) as String?
