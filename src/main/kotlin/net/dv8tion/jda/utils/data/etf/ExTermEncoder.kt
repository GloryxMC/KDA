...
object ExTermEncoder {
    ...
    private fun pack(buffer: ByteBuffer, value: Any?): ByteBuffer {
        if (value is String) return packBinary(buffer, value as String?)
        ...
        if (value is Int || value is Short) return packInt(buffer, value as Int)
        if (value is Long) return packLong(buffer, value)
        if (value is Float || value is Double) return packFloat(buffer, value as Double)
        ...
        throw UnsupportedOperationException("Cannot pack value of type " + value.javaClass.name)
    }
    ...
    private fun countBytes(value: Long): Byte {
        ...
        return Math.ceil((64 - leadingZeros) / 8.0).toByte()
    }
}
