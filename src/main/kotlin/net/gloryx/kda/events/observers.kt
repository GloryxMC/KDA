...
): Message = jda.await<MessageReceivedEvent> {
    it.channel == this
 ...
 */
suspend inline fun MessageChannel.awaitButton(
        id: String,
        user: User? = null,
        crossinline filter: (ButtonInteractionEvent) -> Boolean = { true }) =
        jda.await<ButtonInteractionEvent> {
            it.channel == this
                    && it.componentId == id
